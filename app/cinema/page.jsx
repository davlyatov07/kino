	...
	return (
		<>
			...
			<section className='text-white bg-black'>
				<div className='absolute h-full -inset-1 bg-gradient-to-t from-transparent via-black/20 to-black'></div>
				<div className='h-screen'>
					<Image
						src={product.bigimg}
						className='pt-10 object-cover h-full object-top'
						alt={product.name}
					/>
				</div>
				<div className='absolute h-[705px] inset-0 bg-gradient-to-bl from-transparent via-black/50 to-black'></div>
				<div className='absolute -bottom-5 flex flex-col items-start justify-end h-full p-6 md:p-10'>
				...
				<div className='mt-20 pl-10 flex flex-col '>
					<h1 className='text-white text-3xl font-bold зе-10'>
						{visibleContent === 'trailer' ? 'Трейлер' : 'Фильм'}
					</h1>
					<iframe
						className='rounded-3xl md:w-full w-96 md:h-72 md:max-w-lg mt-6'
						src={visibleContent === 'trailer' ? product.trailer : product.film}
						title={`${product.name} - ${
							visibleContent === 'trailer' ? 'Трейлер' : 'Фильм'
						}`}
						frameBorder='0'
				...
				<div className='flex md:flex-row flex-col md:gap-0 gap-10 pt-16 items-center justify-between px-10'>
					...
					{/* Блок с рейтингом */}
					<div className='flex border rounded-3xl  bg-neutral-800 p-6'>
						<h1 className='text-5xl border-r  h-28 pr-4 text-purple-700 font-bold'>
							{product.grade}
						</h1>
